...
class Sydney_Employees_B extends WP_Widget {
	...
	public function __construct() {
        ...
        parent::__construct(false, $name = __('Sydney FP: Employees_B', 'sydney'), $widget_ops);
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['sydney_employees_b']) )
			delete_option('sydney_employees_b');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'sydney_employees_b', 'widget' );
		}
...
<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'sydney_employees_b', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
